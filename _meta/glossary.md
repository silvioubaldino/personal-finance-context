...
| **Transfer** _(Transferência)_ | Movimentação de valor entre duas `Wallet`s do mesmo usuário. Não é uma entidade própria: é modelada como duas `Movement`s ligadas por `pair_id` (uma saída, uma entrada) e não entra no resultado de entradas/saídas do período. | "Movement entre carteiras" |
